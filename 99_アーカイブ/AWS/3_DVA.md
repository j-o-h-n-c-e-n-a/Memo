...
### Q. プロビジョンドスループットを効率的に使用するためのAmazon DynamaDB パーティションキーのスキーマとして適切なのはどれですか
...
### Q. あるセキュリティシステムではカメラ3000台を監視し、画像メタデータを30秒ごとにAmazon DynamaDB テーブルに送信しています。各サンプルについて512バイトのデータ書き込みが行われます。書き込みは等間隔に分散して行われるものとします。ターゲットテーブルに求められる書き込みスループットはどのくらいですか
1. ３０書き込みキャパシティ
2. １００書き込みキャパシティ
3. ６００書き込みキャパシティ
4. ３００書き込みキャパシティ
...
### Q. DynamoDBで大規模なScanオペレーションを実行する場合、テーブルのプロビジョンドスループットがスキャンによって受ける影響を最小限に抑えるための手法として有効なものはどれですか
...
### Q. 企業Bでは、オンライン画像認識サービスを提供しており、スケーラビリティを実現するためにＳＱＳを利用してシステムコンポーネントを切り離しています。SQSコンシューマは、画像キューを可能な限り頻繁にポーリングして、エンドツーエンドのスループットを可能な限り高く維持することができます。しかし、同社では短い周期でのポーリングによってCPUサイクルが浪費され、空のレスポンスによりコストが増加しています。企業Bが空のレスポンスの数を減らすにはどうすればよいですか
...
### Q. Elastic Load Balancingサービスを使用してトラフィックを複数のウェブサーバーに分散させるアプリケーションを記述しました。ユーザーから、ログイン後、アプリケーションを使用している最中に、再度ログインを求められることがあるという不満が出ています。アプリケーションが想定外の動作をしているようです。これを防ぐにはどうすればよいですか
...
### Q. 以下のサービスのうち、キーバリューストアに該当する者はどれですか
1. Redis
2. SQS
3. DynamoDB
4. SWS
...
### Q. アプリケーションではユーザーの状態情報を保存する必要があります。どのAWSサービスを使用するのが適切ですか
1. DynamoDB
2. RDS
3. X-Ray
4. SNS
...
### Q. キャッシュに使用できるのはどのAmazonサービスですか
1. S3
2. CloudFront
3. Elasticsearch
4. Elasticahe
...
### Q. ソリューションのリファクタリングに役立ち、オンプレミスでホストされる既存のアプリケーションのコスト削減を可能にするサービスはどれですか
1. 静的ウェブサイトのホスティング
...
### Q. CPU使用率が60%を超えたときにトリガーされるようにCloudWatchアラームを設定しました。CPUの使用率は現在80%です。このとき、アラームのステータスは次の内どれですか
1. OK
2. ALARM
3. INSUFFICIENT DATA
4. 不十分なCPU
...
### Q. アプリケーションで６GBのファイルをS3にアップロードしようとしていますが、「指定されたアップロードは、許容される最大オブジェクトサイズを超えています。」というメッセージが表示されます。この問題を解決する方法はありますか
...
### Q. 次のAWSエラーの意味について正しく説明している分は以下の内どれですか。「InternalError:内部エラーが発生しました。後でもう一度お試しください」
...
### Q. DynamoDBテーブルを使用するコードを記述していて、”ProvisionedThroughputExceededException”という例外を受信しました。しかし、テーブルのCloudWatchメトリクスによると、プロビジョニングされたスループットを超えていません。どのような原因が考えられますか
...
### Q. CloudFormation スタックのいずれかのリソースを作成できない場合、デフォルトではどうなりますか
...
### Q. エラーコード４０４ NOT FOUND：NoSuchBucketの応答を受信しました。これはどのような内容のエラーコードですか
