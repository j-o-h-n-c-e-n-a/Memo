...
<details><div>
    答え：2
    Azure Machine Learning を使用すると、機械学習モデルのデータ準備、トレーニング、検証、デプロイを管理できます。 Scikit-Learn、PyTorch、Tensorflow などの既存のフレームワークをサポートしており、機械学習をクラウド上で運用できるようにするためのクロスプラットフォームのプラットフォームを提供するものです。
</div></details>
...
<details><div>
    答え：3
    実験の実行でメトリックを記録するには、Run.log* メソッドを使用します。
</div></details>
...
### Q. 
    1. 実験を実行するために Azure Machine Learning Python SDK を使用しています。 Conda 構成 (.yml) ファイルから環境を作成する必要があります。 Environment クラスのどのメソッドを使用する必要がありますか?
1. create
2. from_conda_specification
3. from_existing_conda_environment
<details><div>
    答え：２
    構成ファイルから環境を作成するには、from_conda_specification メソッドを使用します。 create メソッドでは、conda パッケージと pip パッケージを明示的に指定する必要があり、from_existing_conda_environment にはコンピューター上の既存の環境が必要です。
</div></details>

### Q. 
    2. 画像処理装置 (GPU) を必要とするトレーニング実験用のコンピューティング先を作成する必要があります。 必要に応じて複数のノードが自動的に開始されるように、コンピューティングをスケーリングできるようにすることを望んでいます。 どのような種類のコンピューティング先を作成する必要がありますか?
1. コンピューティング インスタンス
2. コンピューティング クラスター
3. 推論クラスター
<details><div>
...
### Q. 
    1. 2 つのステップを含むパイプラインを作成しているとします。 ステップ 1 ではいくつかのデータが前処理され、ステップ 2 では前処理されたデータを使用してモデルがトレーニングされます。 ステップ 1 からステップ 2 にデータを渡し、これらのステップの間に依存関係を作成するために使用するオブジェクトの種類を指定してください。
1. データストア
2. OutputFileDatasetConfig
3. データセット
<details><div>
    答え：２
    パイプライン内のステップ間でデータを渡すには、OutputFileDatasetConfig オブジェクトを使用します。
</div></details>

### Q. 
    2. 毎週実行するパイプラインを発行しました。 Schedule.create メソッドを使用してスケジュールを作成することを計画しているとします。 パイプライン実行の頻度を構成するために最初に作成する必要のあるオブジェクトの種類を指定してください。
1. データストア
2. PipelineParameter
3. ScheduleRecurrence
<details><div>
    答え：３
...
### Q. 
    1. Azure Machine Learning 用の Python SDK を使用して、モデルをトレーニングしました。 高いスケーラビリティとセキュリティを備え、コンテナー化されたリアルタイム サービスとしてモデルをデプロイする必要があります。 このサービスをホストするには、どのような種類のコンピューティングを作成する必要がありますか?
1. Azure Kubernetes Service (AKS) 推論クラスター
2. GPU を使用するコンピューティング インスタンス
3. 複数のノードがあるトレーニング クラスター。
<details><div>
    答え：1
    モデルをスケーラブルで、セキュリティで保護され、コンテナー化されたサービスとしてデプロイするには、AKS クラスターを使用する必要があります。
</div></details>

### Q. 
    2. モデルをリアルタイムの推論サービスとしてデプロイしています。 サービスのエントリ スクリプトには、どのような関数が含まれている必要がありますか?
1. main() と score()
2. base() と train()
3. init() と run()
<details><div>
...
### Q. 
    1. あなたは大量のデータ ファイルの新しい値を予測するために使用するバッチ推論パイプラインを作成中ですか? あなたはパイプラインによってスコアリング スクリプトが複数のノード上で実行され、その結果が照合されるようにしたいと考えています。 パイプラインにはどのような種類のステップを含める必要がありますか?
1. PythonScriptStep
2. ParallelRunStep
3. AdlaStep
<details><div>
...
<details><div>
    答え：3
     append_row 出力アクションを使用すると、ParallelRunStep ステップからの結果が parallel_run_step.txt という名前のファイル内で照合されます。
</div></details>
...
<details><div>
    答え：1
    差分プライバシー ソリューションでは、集計が統計的に一貫しているが非決定的となるように、分析の生成時にデータにノイズが追加されます。また、集計に対する個人の影響を特定することはできません。
</div></details>
...
### Q. 
    1. 大学の学位プログラムの入学承認決定をサポートするために、二項分類モデルをトレーニングしています。 このモデルが公平であり、民族性に基づいて差別されていないかどうかを評価するにはどうすればよいですか?
1. 検証データセットを使用して各トレーニング済みモデルを評価し、正確性スコアが最も高いモデルを使用します。 正確なモデルは本質的に公平です。
2. トレーニング データセットから民族性の特徴を取り除きます。
3. 民族間の選択率とパフォーマンスメトリックの不一致を比較します。
<details><div>
    答え：3
     民族性をセンシティブ フィールドとして使用し、民族性の値ごとに選択率とパフォーマンス メトリックの不一致を比較することで、モデルの公平性を評価することができます。
</div></details>

### Q. 
    2. ノートブックでモデルを評価するために Fairlearn を使用しました。 Azure Machine Learning ワークスペースでモデルを登録します。 Azure Machine Learning スタジオでモデルを選択し、そこから公平性ダッシュボードを表示して、パフォーマンス メトリックの不一致を比較できるようにする必要があります。 何をする必要がありますか?
1. モデルのダッシュボード メトリックをアップロードする実験を実行します。
2. ノートブックを Azure Machine Learning ワークスペースに保存します。
3. selection_rate_group_summary 関数を使用して公平性データを取得し、Azure Machine Learning ワークスペースにファイル データセットとして保存します。
<details><div>
    答え：1
    upload_dashboard_dictionary 関数を使用して、ダッシュボードのメトリックをアップロードします。 次に、モデルからアクセスできる Azure Machine Learning スタジオの実験の実行ページでダッシュボードを表示できます。
</div></details>

### Q. 
    3. あなたは、候補者が雇用の役割で成功するかどうかを予測する二項分類子に最適なモデルを見つけるために、グリッド検索の緩和手法を使用する予定です。 このモデルでは、性別の特徴の各カテゴリから同数の候補者を選択するようにしたいと考えています。 どの一致制約を使用する必要がありますか?
1. 人口統計の一致
2. エラー率の一致。
3. 有界グループ損失。
<details><div>
    答え：1
    人口統計の一致は、性別カテゴリ間の選択率の不一致を最小限に抑えようと試みるものです。
</div></details>

### Q. 
    1. あなたは Azure Kubernetes Service (AKS) クラスターに、モデルをリアルタイムの推論サービスとしてデプロイしました。 このサービスのテレメトリを取り込んで分析するには、何を行う必要がありますか?
1. モデルを ACI サービスとして再デプロイする。
2. Application Insights を有効にする。
3. AKS クラスターを Azure Machine Learning ワークスペースと同じリージョンに移動する。
<details><div>
    答え：２
    Application Insights を介したテレメトリ分析を有効にするには、サービスに対して Application Insights を有効にする必要があります。
</div></details>

### Q. 
    2. あなたは推論サービスのテレメトリにカスタム情報を含め、それを Application Insights を使用して分析したいと考えています。 サービスのエントリ スクリプト内で何を行う必要がありますか?
1. Run.log メソッドを使用して、カスタム メトリックをログする。
2. カスタム メトリックを ./outputs フォルダーに保存する。
3. print ステートメントを使用して、STDOUT ログにメトリックを書き込む。
<details><div>
    答え：３
    カスタム メトリックを含めるには、スコアリング スクリプトに print ステートメントを追加して、カスタム情報が STDOUT ログに書き込まれるようにします。
</div></details>

### Q. 
    1. 昨年収集されたデータを含むデータセットを使用してモデルをトレーニングしました。 今年は、新しいデータを収集する予定です。 モデルのパフォーマンスに影響する可能性のある、変化するデータの傾向を追跡したいと考えています。 何をする必要がありますか?
1. 既存のトレーニング データセットの新しいバージョンで新しいデータを収集し、両方のデータセットをプロファイルします。
2. 別のデータセットで新しいデータを収集し、トレーニング データセットをベースラインとして、新しいデータセットをターゲットとして使用するデータ ドリフト モニターを作成します。
3. トレーニング データセットを、元のトレーニング データと新しいデータの両方を含む新しいデータセットに置き換えます。
<details><div>
    答え：２
     変化するデータの傾向を追跡するために、トレーニング データをベースラインとして、新しいデータをターゲットとして使用するデータ ドリフト モニターを作成します。
</div></details>

### Q. 
    2. あなたはデータ ドリフト モニターを作成しています。 データ分布に大きな変化が検出された場合、データ サイエンス チームに自動的に通知したいと考えています。 どうすればよいでしょうか。
1. AlertConfiguration を定義し、drift_threshold 値を設定します。
2. データ サイエンティストが新しいデータを確認する時間を確保できるように、データ ドリフト モニターの待機時間を設定します。
3. データ サイエンス チームのメール アドレスをタグとして含め、トレーニング データセットをモデルに登録します。
<details><div>
    答え：１
     データ ドリフトについてオペレーターに通知するには、通知先のメール アドレスと、通知をトリガーする変化のレベルを定義する誤差のしきい値を指定した AlertConfiguration を作成します。
</div></details>
...
<details><div>
    答え：１
    カスタム役割を作成することにより、Actions と NotActions を使用してアクセス権を許可および制限して、ロールを特定のタスクに使用できるようにすることができます。
</div></details>
...
<details><div>
    答え：３
    シークレットは、API キー、パスワード、証明書、暗号化キーなど、アクセスを厳重に制御する必要があるものです。
</div></details>
...
<details><div>
    答え：２
    ポイント対サイト、サイト間、ExpressRoute などの VPN は、インターネット接続を暗号化するか、またはインターネットをまったく経由しないため、リソースへのアクセスは制限されます。
</div></details>
