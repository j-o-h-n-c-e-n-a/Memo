...
* init
    + /etc/rc スクリプトが、代替の初期化デーモンを指定します
    + ランレベルで起動するスクリプトが異なる（/etc/rc1.d, rc2.d, ...）
* SysVinit
    + あらかじめ決められた順序で起動する
* systemd
...
* uname
    + システムのハードウェア・アーキテクチャ、Linux カーネル・バージョン、ディストリビューション、およびディストリビューション・リリースを表示します。
* history
    + 以前に実行したコマンドを印刷または再利用する。
* .bash_history
* quoting

### 演習
#### 1-1. man システムを使って、apropos に簡単なコマンドを出力するように指示し、簡単な使用法のメッセージだけを出力して終了させる方法を決定してください。
<details><div>
    man apropos を実行し、「オプション」セクションを --usage の段落までスクロールダウンしてください。
</div></details>

#### 1-2. grep コマンドにどのような著作権ライセンスが割り当てられているかを調べるには、man システムを使用します。
<details><div>
    man grepを実行し、文書の「著作権」セクションまでスクロールダウンしてください。このプログラムでは、Free Software Foundation の著作権が使用されていることに注意してください。
</div></details>

#### 2-1. お使いのコンピュータで使用されているハードウェアアーキテクチャと Linux カーネルバージョンを、読みやすい出力形式で特定します。
    man uname を実行し、「説明」セクションを読み、あなたが望む正確な結果のみを表示するコマンド引数を確認します。v はカーネルバージョンを、-i はハードウェアプラットフォームを提供することに注意してください。
<details><div>
    man uname
    uname -v
    uname -i
...
#### 2-2. 動的履歴データベースと .bash_history ファイルの最後の 20 行を出力して比較します。
    動的履歴データベースと .bash_history ファイルの最後の 20 行を出力して比較します。
<details><div>
    history 20
    tail -n 20 .bash_history
</div></details>

#### 2-3. apropos ツールを使って、接続されている物理ブロックデバイスのサイズをメガバイトやギガバ イトではなく、バイト単位で表示するために必要なコマンドのある man ページを特定します。
    一つの方法は、文字列 block で apropos を実行し、その結果を読んで、lsblk がブロックデバイスをリストしている (したがって、このニーズに最も適したツールである) ことを確認し、man lsblk を実行して、「説明」セクションをスクロールして、-b がデバイスサイズを bytes で表示することを確認する、というものでしょう。最後に、lsblk -bを実行して、何が出てくるか見てください。
<details><div>
    apropos block
    man lsblk
    lsblk -b
...
### 主な知識分野:
* テキストファイルと出力ストリームをテキストユーティリティのフィルターを通して送信し、GNU textutils パッケージにある標準的な UNIX コマンドを使って出力を変更します。

### ファイル、用語、およびユーティリティ
    以下は、使用されるファイル、用語、およびユーティリティの部分的なリストです。
* cat
    + プレーンテキストファイルの結合や読み取りに使用します。
    + テキストをパイプする
* bzcat
    + bzip2方式で圧縮されたファイルの処理または読み込みを可能にする。
* xzcat
    + xz方式で圧縮されたファイルの処理・読み込みを可能にする。
* zcat
    + gzip方式で圧縮されたファイルの処理、または読み込みを許可する。
    + gzip は、元のファイルを削除することに注意
* less
    + ファイルの内容をページ分割し、ナビゲーションと検索機能を提供するコマンドです。
* head
    + 最初の10行を読むのに使われる
    + n スイッチを使用すると、より少ない行数またはより多くの行数を表示できます。
* tail
    + 最後の10行を読むのに使われる
    + n スイッチを使うと、より少ない行数でも表示させることができる。
    + f オプションは、テキスト・ファイルに新しいデータが書き込まれたときに、その出力を追跡するために使用する。
* wc
    + 「ワードカウント」の略。パラメータによって、文字数、単語数、行数をカウントする。
* nl
...
<details><div>
    BashはHeredoc入力モードに入り、ピリオドが1行に1つ現れると終了します。入力されたテキストはstdoutにリダイレクトされます（画面に出力されます）。
</div></details>
...
### 主な知識分野:
* MBR および GPT パーティションテーブルの管理
    + MBRは、ＭＳ－ＤＯＳの名残
    + MBRには、2TBを超えるサイズのディスクを扱うことができない、1つのディスクに4つのプライマリパーティションしかない、といった一連の制限がある
    + MBRには、拡張パーティションに論理パーティションを5つ置くことができる
    + Linuxには、プライマリーパーティションと拡張パーティションという違いの利点がありません。
    + GPTには、最大128個のパーティションをもつことができるので、拡張パーティションは必要ない
* 様々なmkfsコマンドを使用して、以下のような様々なファイルシステムを作成することができます。
* ext2/ext3/ext4
* XFS
    + 高性能なファイルシステム
* VFAT
    + 長いファイル名（255文字まで）をサポートするFAT16形式の拡張版です。
* exFAT
    + 最大ファイルサイズは16エクサバイト（FAT32の4GBから）、最大ディスクサイズは128ペタバイトです。
* マルチデバイスファイルシステム、圧縮、サブボリュームを含むBtrfsの基本機能知識
    + マルチデバイスのサポート（RAIDセットアップのようなストライピング、ミラーリング、ストライピング+ミラーリングを含む）、透過的圧縮、SSD最適化、増分バックアップ、スナップショット、オンラインデフラグ、オフラインチェック、サブボリュームのサポート（クォータ付き）、重複排除、その他多数
    + クラッシュにも非常に強い
### ファイル、用語、およびユーティリティ
    以下は、使用されるファイル、用語、およびユーティリティの部分的なリストです。
* fdisk
...
* gdisk
    + MBR パーティションを作成、削除する
* parted
    + MBR と GPT のディスクにパーティションを作成、削除、回復する
* mkfs
    + ファイルシステムを作成（XFS, BTRF など）
    + mkfs.xfs TARGET：XFS 
* mkswap
    + スワップパーティションとスワップファイルの作成、セットアップ、有効化
* mklabel
    + 空のディスクにパーティションテーブルを作成する

## 104.2 ファイルシステムの整合性を維持する
    最近のLinuxのファイルシステムはジャーナル化されています。
    これは、すべての操作が実行される前に内部ログ（ジャーナル）に登録されることを意味します。
    ファイルシステムの使用状況を監視し、その動作を最適化するために利用できるツール、および損傷の確認と修復の方法について
* 重さ: 2
* 形容：候補者は、標準ファイルシステムとジャーナリングファイルシステムに関連する追加データを維持できる必要があります。
### 主な知識分野:
* ファイルシステムの整合性を検証する。
    + ファイルシステムの使用領域と空き領域をチェックする方法
    + tune2fs を使って ext ファイルシステムを微調整する
* 空き領域とinodeを監視する
    + df の出力をあなたのニーズに合わせて調整する方法
* ファイルシステムの簡単な問題を修復する。
    + fsck と e2fsck を使ってファイルシステムの整合性をチェックし、修復する
    + xfs_repair を使って XFS ファイルシステムをチェックし、修復する
### ファイル、用語、およびユーティリティ
    以下は、使用されるファイル、用語、およびユーティリティの部分的なリストです。
* du
    + ファイルシステムで使用中のディスク容量を表示します。
* df
...
* umask
    + ファイルとディレクトリのデフォルトのパーミッションマスクを照会、または設定します。
* chown
    + ファイルまたはディレクトリの所有ユーザーおよび/またはグループを変更します．
* chgrp
    + ファイルまたはディレクトリの所有グループを変更する．
 
## 104.6 ハードリンクとシンボリックリンクの作成と変更
    リンクとは何か、そしてリンクをどのように管理するかについて
* 重さ: 2
...
* ハードリンクとソフトリンクの識別
    + シンボリックリンク
        - ソフトリンクとも呼ばれ、他のファイルのパスを指し示す。
        - パーティションとファイルシステムを横断できる
        - リンクの指すファイル（ターゲットと呼ばれる）を削除してもリンクは存在しますが、「何も指していない」ことになるので「動かなくなる」のです。
    + ハードリンク
        - 元のファイルの 2 番目の名前と考えることができます。
        - 複製ではなく、ディスク上の同じ場所（inode）を指すファイルシステム内の追加エントリです。
        - 同じパーティションとファイルシステム上にしか存在できない
* ファイルのコピーとリンク
    + ハードリンク
        - 通常のファイルと同じように扱われるため、ファイル名を変更したり、システム上で移動したりできます
        - リンク先は変わらないので、リンク切れを心配せずに利用できる
    + ソフトリンク
        - 相対パスを使用している場合、リンクが壊れる可能性がある
* システム管理作業をサポートするためにリンクを使用する
### ファイル、用語、およびユーティリティ
    以下は、使用されるファイル、用語、およびユーティリティの部分的なリストです。
* ln
* ls
...
## 104.7 システムファイルを検索し、ファイルを正しい場所に配置する
    Linux システムでファイルを見つける方法について
* 重さ: 2
* 形容： 受験者は、一般的なファイルの場所やディレクトリ分類など、ファイルシステム階層標準(FHS)に精通している必要があります。
### 主な知識分野:
* FHSの下でのファイルの正しい位置を理解する。
    + /
        - ルートディレクトリと呼ばれる、階層の最上位に位置するディレクトリです。他のすべてのディレクトリは、このディレクトリの中にあります。ファイルシステムはよく「木」にたとえられますが、これはすべての枝がつながっている「幹」のようなものです。
    + /bin
        - すべてのユーザーが利用できる基本的なバイナリ。
    + /boot
        - Initial RAM Disk (initrd)やLinuxカーネルなど、ブートプロセスに必要なファイルです。
    + /dev
        - デバイス・ファイル。これらは、システムに接続された物理デバイス（例えば、/dev/sdaは最初のSCSIまたはSATAディスクになります）またはカーネルによって提供される仮想デバイスのいずれかであることができます。
    + /etc
        - ホスト固有の設定ファイルです。プログラムは必要に応じて/etcの下にサブディレクトリを作成し、複数の設定ファイルを格納することができます。
    + /home
        - システム内の各ユーザーは、個人的なファイルや設定を保存するための「ホーム」ディレクトリを持っており、そのほとんどは/home以下に配置されます。通常、ホームディレクトリはユーザー名と同じになるので、ユーザーJohnは/home/johnの下に自分のディレクトリを持つことになります。例外はスーパーユーザー（root）で、これは別のディレクトリ（/root）を持ち、一部のシステムユーザーです。
    + /lib
        - OSの起動や、/binや/sbinの下のバイナリを実行するのに必要な共有ライブラリ。
    + /media
        - フラッシュドライブ、CD/DVD-ROMリーダー、フロッピーディスク、メモリーカード、外付けディスクなど、ユーザーがマウント可能なリムーバブルメディアは、この下にマウントされます。
    + /mnt
