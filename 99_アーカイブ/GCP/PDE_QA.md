...
### Q. 質問4: 未回答
住宅価格を予測するモデルを作成しています。予算の制約により、リソースに制約のある 1 台の仮想マシンで実行する必要があります。どの学習アルゴリズムを使うべきか?
1. 線形回帰
2. 
3. 
4. 
<details><div>
    答え：1
説明
予算の制約があり、リソースに制約のある 1 つの仮想マシンで住宅価格予測モデルを実行する必要がある場合、検討すべき最適な学習アルゴリズムは次のとおりです。
A. 線形回帰
このシナリオで線形回帰が適している理由は次のとおりです。
リソース効率: 線形回帰は、最も単純な機械学習アルゴリズムの 1 つであり、計算量が軽量です。大量の計算リソースやメモリを必要としません。リソースに制約のある仮想マシンで実行することは可能です。
解釈可能性: 線形回帰は、明確で解釈可能なモデルを提供します。入力フィーチャ (面積、寝室数など) と予測住宅価格の関係を簡単に理解できます。これは、モデルの予測を説明するのに役立ちます。
予算の制約: 予算の制約について言及したので、線形回帰は通常、ニューラル ネットワークなどのより複雑なアルゴリズムと比較して、トレーニングと推論に必要な計算能力が少ないことに注意することが重要です。
ニューラル ネットワーク (フィードフォワードとリカレントの両方) は予測モデリングには強力ですが、計算負荷が高く、GPU や TPU などの特殊なハードウェアにアクセスできない限り、リソースに制約のある環境には適さない場合があります。
ロジスティック分類 (オプション B) は、住宅価格の予測などの回帰問題ではなく、主に二項分類タスクに使用されます。
</div></details>

### Q. 質問5: 未回答
Google BigQuery ストリーミング挿入を使用して、会社の新しいリアルタイム データ ウェアハウスを構築しています。データは複数回送信される場合がありますが、各行には一意の ID とイベントのタイムスタンプがあります。対話形式でデータを照会するときに、重複する行が含まれないようにする必要があります。
どのクエリタイプを使用する必要がありますか?
1. 
2. 
3. 
4. 一意のIDによるPARTITIONとWHERE行が1に等しいROW_NUMBERウィンドウ関数を使用します。
<details><div>
    答え：4
説明
D. 一意のIDによるPARTITIONとWHERE行が1に等しいROW_NUMBERウィンドウ関数を使用します。
オプション D では、ROW_NUMBER ウィンドウ関数を使用して、一意の ID で定義されたパーティション内の各行に一意の行番号を割り当てます。次に、行番号が 1 に等しい行のみを含むように結果をフィルター処理します。これにより、重複を除外しながら、各パーティション内の最初の行が効果的に選択されます。
</div></details>
...
### Q. 質問15: 未回答
会社のオンプレミスの Apache Hadoop サーバーは、その寿命の終わりに近づいています。IT 部門は、クラスタを Google Cloud Dataproc に移行することを決定しました。クラスタを同種に移行するには、ノードあたり 50 TB の Google Persistent Disk が必要です。最高情報責任者 (CIO) は、その容量のブロック・ストレージを使用するコストを懸念しています。移行のストレージ コストを最小限に抑える必要があります。
これを達成するには、どのような手順を踏むことができますか?
1. Google Cloud Storage にデータを入れます。
2. 
3. 
4. 
<details><div>
...
Google Cloud Storage は、特にコールドデータやアクセス頻度の低いデータに対して、費用対効果の高いストレージ ソリューションです。データを Google Cloud Storage に移行することで、すべてのデータに永続ディスクを使用する場合と比較してコストを削減できます。
このオプションが正しい選択である理由は次のとおりです。
コスト削減: Google Cloud Storage は、通常、特に高パフォーマンス ストレージを必要としないデータの場合、永続ディスクを使用するよりも費用対効果が高くなります。
スケーラビリティ: Google Cloud Storage は拡張性が高く、ストレージ ハードウェアのプロビジョニングや管理を必要とせずに大量のデータを保存できます。
コンピューティングとストレージの分離: データに Google Cloud Storage を使用することで、Dataproc クラスタ内のコンピューティング リソースからストレージを切り離すことができます。この分離により、コスト削減と柔軟性が得られます。
代替オプションの説明 -
オプション D(一部のコールド データを Google Cloud Storage に移行する)も有効なアプローチですが、Google Cloud Storage に移動するデータを選択的に選択する必要があるのに対し、オプション A ではすべてのデータを Google Cloud Storage に移動する必要があります。これらのオプションの選択は、特定のデータ アクセス パターンとコストに関する考慮事項によって異なる場合があります。
</div></details>
...
3. 
4. 
5. 組み込み関数を使用して列 DT の文字列を TIMESTAMP 値にキャストしながら、テーブルCLICK_STREAMのすべての行を返すクエリを作成します。
<details><div>
    答え：5
説明
...
### Q. 質問23: 未回答
特定の日に雨が降るかどうかを予測する機械学習モデルを構築しています。モデルには何千もの入力特徴があり、モデルの精度への影響を最小限に抑えながら、これらの特徴の一部を削除することで、モデルの学習速度を向上させたいと考えています。
その他のオプション
1. 
2. 共依存性の高い特徴を 1 つの代表的な特徴量に結合します。
3. 
4. 
<details><div>
    答え：2
説明
オプション B. 共依存性の高い特徴を 1 つの代表的な特徴量に結合します。
このオプションは、共依存性の高い特徴を1つの代表的な特徴量に結合することが、情報を維持しながら特徴量の次元を減らすための効果的な手法であることを示唆しています。この手法は、特徴量エンジニアリングや次元削減で一般的に使用され、情報を大幅に失うことなくモデルの効率を向上させます。
</div></details>

### Q. 質問24: 未回答
会社では、Google Cloud Dataflow を使用して、学習アルゴリズムのデータを前処理しています。データの前処理ステップでは、チームが分析したい多数のデータログが生成されます。ただし、キャンペーンの動的な性質により、データは毎時間指数関数的に増加しています。 データ サイエンティストは、ログ内の新しい主要機能のデータを読み取るために、次のコードを記述しました。BigQueryIO.Read (英語) .named("ReadLogData") .from("clouddataflow-readonly:samples.log_data") このデータ読み取りのパフォーマンスを向上させる必要があります。あなたは何をするべきか?
1. 
2. .fromQuery 操作を使用して、テーブルから特定のフィールドを読み取ります。
3. 
4. 
<details><div>
    答え：2
説明
正解 - B. .fromQuery 操作を使用して、テーブルから特定のフィールドを読み取ります。
BigQueryIO.read.from() は、BigQuery からテーブル全体を直接読み取ります。この関数は、テーブル全体を Google Cloud Storage の一時ファイルにエクスポートし、後でそこから読み取られます。これはエクスポート ジョブのみを実行し、後で Dataflow は(BigQuery からではなく)GCS から読み取るため、ほとんど計算を必要としません。
BigQueryIO.read.fromQuery() はクエリを実行し、クエリ実行後に受け取った結果を読み取ります。したがって、この関数は、クエリが最初に実行される必要があることを考えると、より時間がかかります (対応する経済的および計算コストが発生します)。
参考リンク -
...
### Q. 質問37: 未回答
現在 Google Cloud Platform(GCP)で実行されているデータパイプラインのセキュリティのベストプラクティスを実装しています。パイプラインは、次の手順で構成されます。
非公開情報を含む夜間バッチファイルは、Google Cloud Storage(GCS)から取得されます。
バッチ ファイルは、Google Cloud Dataproc クラスタ上の Spark Scala ジョブで処理されます。
処理の結果はGoogle BigQueryに保存されます。
これらのジョブを自動化して、プロジェクト所有者として手動で実行する必要がなくなります。このワークロードを安全に実行するにはどうすればよいでしょうか。
1. 
2. 
3. バッチ ファイルの読み取りと BigQuery への書き込みが可能なサービス アカウントを使用します。
4. 
<details><div>
    答え：3
説明
このワークロードを安全に実行するための正しいアプローチは次のとおりです。
C. バッチ ファイルの読み取りと BigQuery への書き込みが可能なサービス アカウントを使用します。
以下にその説明を示します。
サービス アカウント: サービス アカウントの使用は、Google Cloud でのジョブを自動化する安全な方法です。サービスアカウントには必要な権限のみを付与できるため、プロジェクトオーナーなどの高い特権を持つアカウントの使用に伴うリスクを軽減できます。
権限: サービス アカウントには、Google Cloud Storage からのバッチ ファイルの読み取りや BigQuery への結果の書き込みなど、このジョブに必要な特定の権限を付与する必要があります。これにより、サービス アカウントには、不要なアクセス許可なしで必要なアクセス権が付与されます。
正しくないオプション -
オプション A は、Google Cloud Storage バケットを自分だけに制限すると、自動化とコラボレーションが妨げられる可能性があるため、推奨されません。
オプション B のプロジェクト所有者にサービス アカウントを付与することは、セキュリティの観点から推奨される方法ではありません。プロジェクトオーナーの権限が多すぎるため、可能な限り避ける必要があります。
オプション D では、プロジェクト閲覧者ロールを持つユーザー アカウントを使用しますが、自動化には適していません。ユーザー アカウントは人間のユーザーを対象としており、自動化されたタスクへのアクセス権を付与すると、セキュリティと運用上の問題が発生する可能性があります。
</div></details>

### Q. 質問38: 未回答
グローバルに分散されたオークション アプリケーションを使用すると、ユーザーはアイテムに入札できます。場合によっては、ユーザーがほぼ同じ時間に同じ入札を行い、異なるアプリケーションサーバーがそれらの入札を処理します。各入札イベントには、項目、金額、ユーザー、タイムスタンプが含まれます。これらの入札イベントを 1 か所にリアルタイムで照合して、どのユーザーが最初に入札したかを判断します。あなたは何をするべきか?
1. 
2. 
3. 
4. 各アプリケーション サーバーで、入札イベントが発生したら Google Cloud Pub/Sub に書き込みます。
<details><div>
    答え：4
説明
D. 各アプリケーション サーバーで、入札イベントが発生したら Google Cloud Pub/Sub に書き込みます。プル サブスクリプションを使用して、Google Cloud Dataflow を使用して入札イベントをプルします。各項目の入札は、最初に処理された入札イベントでユーザーに渡します。
Google Cloud Pub/Sub: リアルタイムのイベントデータを処理できる、スケーラブルで信頼性の高いメッセージング サービスです。各アプリケーション サーバーは入札イベントを Cloud Pub/Sub に公開できるため、効率的に配信できます。
Google Cloud Dataflow: ストリーム処理に使用され、Cloud Pub/Sub のイベントを使用できます。 Dataflow を使用すると、タイムスタンプを分析して最初に発生した入札イベントを特定するなど、データをリアルタイムで処理できます。
プル サブスクリプション: プル サブスクリプションを使用すると、Cloud Dataflow は、メッセージを処理する準備ができたときに、Cloud Pub/Sub からメッセージを効率的に取得できます。
リアルタイムでの処理: 入札イベントが発生したときに処理することで、タイムスタンプに基づいてどのユーザーが最初に入札したかをすばやく判断できます。
正しくないオプション -
Cloud Pub/Sub への書き込みについて言及しているオプション B は正しいですが、このシナリオでのリアルタイム処理の重要なコンポーネントである Google Cloud Dataflow の使用は指定されていません。
オプションAとCには、大量のデータを効率的にリアルタイム処理するようには設計されていない共有ファイルまたは複数のMySQLデータベースへの書き込みが含まれます。
オプション D は、リアルタイムのイベント データを処理して効率的に処理し、どのユーザーが最初に入札したかを判断する Google Cloud の機能と連携しています。
</div></details>

### Q. 質問39: 未回答
組織は Google BigQuery で 6 か月間データを収集および分析しています。分析されるデータの大部分は、events_partitioned と呼ばれる時間分割されたテーブルに格納されます。クエリのコストを削減するために、組織では、過去 14 日間のデータのみをクエリする events というビューを作成しました。ビューはレガシ SQL で記述されます。来月には、既存のアプリケーションが BigQuery に接続し、ODBC 接続を介してイベント データを読み取る予定です。アプリケーションが接続できることを確認する必要があります。次のアクションのうち、実行するアクションを <> つ選択する必要がありますか?(<>つ選択してください。
1. 
2. 
3. 標準 SQL を使用して、events_partitioned に対する新しいビューを作成します。
4. 認証に使用する ODBC 接続のサービス アカウントを作成します。
<details><div>
    答え：3,4
説明
正解-
C. 標準 SQL を使用して、events_partitioned に対する新しいビューを作成します。既存のビューがレガシー SQL で記述されており、アプリケーションの標準 SQL との互換性を確保する場合は、標準 SQL を使用して新しいビューを作成する必要があります。これにより、標準 SQL を使用するアプリケーションは、ビューを効果的にクエリできます。
D. 認証に使用する ODBC 接続のサービス アカウントを作成します。Google BigQuery に接続するために、ODBC 接続では多くの場合、サービス アカウントによる認証が必要です。実際にサービスアカウントを作成し、ODBC接続に必要な資格情報を提供する必要があります。これにより、ODBC クライアントは BigQuery データを安全に認証し、アクセスできるようになります。
正しくないオプション -
A. 標準 SQL を使用して、イベントに関する新しいビューを作成します。このオプションは、既存の "events" ビューの上に新しいビューを作成することを提案しますが、既存の "events" ビューがレガシー SQL で記述されているという事実には対処していません。標準 SQL で新しいビューを作成しても、既存のビューで使用される基になる SQL ダイアレクトは変更されません。
B. 標準の SQL クエリを使用して、新しいパーティション テーブルを作成します。このオプションでは、標準 SQL を使用して新しいテーブルを作成します。これは一部のシナリオでは有効なアプローチですが、ODBC 接続を使用するアプリケーションとの互換性を確保するという目標に直接対処するものではありません。新しい表に移行するには、古いビューではなく新しい表を照会するようにアプリケーション・ロジックを変更する必要がある場合があります。
E. ODBC 接続と共有「イベント」の Google Cloud Identity and Access Management(Cloud IAM)ロールを作成します。このオプションはアクセス制御に関連し、ビューで使用される SQL ダイアレクトには直接対応しません。Cloud IAM のロールは権限の管理に重要ですが、従来の SQL ベースのビューを ODBC 接続用の標準 SQL に変換する際には役に立ちません。
</div></details>

### Q. 質問40: 未回答
Firebase Analytics と Google BigQuery の無料統合を有効にしました。その結果、Firebase は BigQuery に app_events_YYYYMMDD 形式で新しいテーブルを毎日自動的に作成するようになりました。レガシ SQL を使用して、過去 30 日間のすべてのテーブルをクエリします。
どうすればいいのでしょうか?
1. TABLE_DATE_RANGE機能を使用してください。
2. 
3. 
4. 
<details><div>
    答え：1
説明
A. TABLE_DATE_RANGE機能を使用してください。
TABLE_DATE_RANGE 関数を使用すると、日付範囲を指定し、その範囲内のクエリ テーブルを指定できます。これは、"app_events_YYYYMMDD" などの日付サフィックスを持つ日次テーブルがあり、複数日にわたるデータをクエリする場合に特に便利です。
正しくないオプション -
B. WHERE_PARTITIONTIME 擬似列を使用します。
このオプションは、レガシー SQL には適用されません。WHERE_PARTITIONTIME 擬似列は標準 SQL の機能であり、従来の SQL クエリでは使用できません。
C. WHERE date BETWEEN YYYY-MM-DD と YYYY-MM-DD を使用します。
このオプションでは、標準の SQL 日付フィルターを使用しようとしますが、日付サフィックスを持つ複数のテーブルをクエリするためのコンテキストがありません。BigQuery の Firebase Analytics によって生成される日次テーブルの動的な性質は考慮されていません。
D. SELECT IF を使用します。(日付 >= YYYY-MM-DD および日付 <= YYYY-MM-DD。
このオプションは、日付範囲でデータをフィルタリングしようとしているように見えますが、有効なSQL構文ではありません。また、Firebase Analytics によって生成された複数の日次テーブルに対してクエリを実行する必要性には対応していません。
</div></details>
...
### Q. 質問43: 未回答
患者記録のデータベースを設計し、当初は 50 つの診療所の数百人の患者のデータを格納していました。データベースでは、すべての患者の記録と訪問を表すために <> つのテーブルを使用し、レポートの生成には自己結合を使用しました。サーバー リソース使用率は <>% でした。
現在、データベースには 100 倍の患者記録を保存する必要があります。その結果、レポートに時間がかかりすぎたり、コンピューティング リソースが不足しているためにエラーが発生したりするため、レポートを実行できなくなります。
データ量の増加に対応し、パフォーマンスを向上させるために、データベース設計をどのように調整する必要がありますか?
1. 
2. 
3. マスター患者レコード テーブルを患者テーブルと訪問テーブルに正規化し、自己結合を回避するために必要な他のテーブルを作成します。
4. 
<details><div>
    答え：3
説明
データベースに 100 倍の患者記録を保存する必要があり、現在の設計でパフォーマンスの問題が発生している状況では、データベース設計に対する最も適切な調整は次のとおりです。
C. マスター患者レコード テーブルを患者テーブルと訪問テーブルに正規化し、自己結合を回避するために必要な他のテーブルを作成します。
正規化: データベースを正規化することで、1 つの大きなテーブルを、より小さな関連するテーブルに分割します。これにより、データの冗長性が減り、データの整合性が向上します。この場合、患者情報を患者テーブルに分離し、訪問を訪問テーブルに分離することは、データを格納するためのより構造化された効率的な方法です。
自己結合の回避: 自己結合は、特に非常に大きなデータセットを扱う場合、計算コストが高くなる可能性があります。患者と来院のデータを別々のテーブルに分けることで、自己結合が不要になり、クエリとレポートがより効率的になります。
クエリ パフォーマンスの向上: 正規化されたテーブルは、通常、データベース エンジンで操作しやすく、クエリのパフォーマンスが向上します。テーブルに適切なインデックスを作成して、クエリの実行をさらに最適化できます。
正しくないオプション -
オプション A (データベース サーバーに容量を追加する) は、短期的にはいくらかの緩和をもたらすかもしれませんが、基本的な設計上の問題には対処しません。データが 100 倍に増加すると、スキーマを最適化しない限り、パフォーマンスの問題が引き続き発生する可能性があります。
オプション B (日付範囲に基づいてテーブルを小さなテーブルに分割する) は、パフォーマンスを向上させるための実行可能なオプションですが、この場合はそれほど効果的ではない可能性があります。シャーディングでは通常、データを水平方向に分割するため、管理が複雑になり、自己結合やデータの正規化に関連する問題に完全に対処できない場合があります。
オプション D (診療所ごとにテーブルを小さなテーブルに分割する) はシャーディングの一種であり、負荷の分散に役立ちますが、自己結合と正規化の基本的な問題には対処しません。また、クエリやレポートロジックが複雑になる可能性もあります。
参考リンク -
