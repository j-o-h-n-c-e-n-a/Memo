...
A. ロックダウンの最初の 30 日間のデータを使用してモデルを再トレーニングします。
B. 使用パターンが正常化するまでデータをモニタリングしてから、モデルを再トレーニングします。
C. 過去 30 日間のデータを使用してモデルを再トレーニングします。1 年経過したら、古いモデルに戻します。
D. 過去 30 日間のデータを使用してモデルを再トレーニングします。モデルの入力データに変更がないか継続的にモニタリングするステップを追加してから、モデルを再トレーニングします。
<details><div>
フィードバック
...
A. デベロッパーにノートパソコンの IP アドレスを尋ねて、承認済みネットワーク リストに追加します。
B. 外部 IP アドレスを削除して、内部 IP アドレスに置き換えます。承認済みのリストには、リモートで作業するデベロッパーのノートパソコンの IP アドレスのみを追加します。
C. Identity and Access Management（IAM）のインスタンスのアクセス権限を付与し、デベロッパーが Cloud SQL Auth Proxy を実行して MySQL インスタンスに接続するようにします。
<details><div>
    答え：
D. Identity and Access Management（IAM）のインスタンスのアクセス権限を付与し、セキュリティのためにアクセスを「プライベート サービス アクセス」に変更して、デベロッパーがノートパソコンから Cloud SQL にアクセスできるようにします。
フィードバック
...
A. インターリーブされたテーブルを作成して、国の下に州を保存します。
B. データを非正規化して、対応する国とともに各州ごとの行を作成します。
C. 既存のアーキテクチャを維持しながら、国と州に短い 2 文字のコードを使用します。
D. 国を 1 つのセルのテキストに結合します。たとえば、「country:state1,state2, …」などです。必要な場合はデータを分割します。
<details><div>
正解
A. インターリーブされたテーブルを作成して、国の下に州を保存します。
フィードバック
...
A. BigQuery に移行します。
B. Cloud Spanner に移行します。
C. Cloud SQL for PostgreSQL インスタンスに移行します。
D. PostgreSQL がインストールされているベアメタル マシンに移行します。
<details><div>
正解
B. Cloud Spanner に移行します。
フィードバック
