...
2. 
3. 
4. CloudComposerを使用してバッチジョブの実行を調整、管理する
<details><div>
    答え：
説明
...
1. 　
2. 
3. ドロップアウトやバッチ正規化などの正規化手法を使用して、過学習を防止します
4. 
<details><div>
    答え：
説明
...
1. 　既存のデータセットでCloudVisionAutoMLを使用します
2. 
3. 
4. 
<details><div>
...
## 3
 ...
 ### Q. 質問1: 回答
高可用性が必要な重要なデータを含む BigQuery データセットがあります。コストを最小限に抑え、目標復旧時点 (RPO) を 30 日間にするには、どのようなストレージ、バックアップ、復旧戦略を使用する必要がありますか?
1. 　
2. 
3. 
4. 
...
説明
正しいオプションは D です。
BigQuery データセットをマルチリージョンに設定すると、データが複数のリージョンにレプリケートされ、高可用性が確保されます。
タイムスタンプのサフィックスが付いたテーブルにデータをコピーするスケジュールされたクエリを作成すると、データのバックアップコピーが提供され、緊急の場合は、テーブルのバックアップコピーを使用してデータを復元できます。この戦略は、30 日間の回復ポイント目標 (RPO) の要件を満たし、コストを最小限に抑えます。
BigQuery データセットをリージョンに設定すると、データが 1 つのリージョンにのみ保存され、リージョンで障害が発生した場合にデータが失われるリスクが高まるため、オプション A は正しくありません。
オプション B は、タイムスタンプのサフィックスが付いたテーブルにデータのコピーを作成する必要があり、追加のストレージ コストが発生する可能性があり、スケジュールされたクエリはデータに対するすべての変更をキャプチャできないため、信頼できるバックアップ ソリューションではないため、正しくありません。
オプション C は、緊急時に特定の時点のスナップショットを使用してデータを回復すると、最新ではない可能性がある特定の時点までのデータのみをキャプチャするため、データが失われる可能性があるため、正しくありません。
</div></details>
...
したがって、オプション C は、変更を日次在庫移動テーブルにストリーミングし、履歴在庫残高テーブルに結合するビューで残高を計算し、在庫残高テーブルを毎晩更新するため、最適なソリューションです。このアプローチにより、ダッシュボードはクエリのパフォーマンスを維持しながら、最新かつ正確なデータにアクセスできます。
 ...
 ### Q. 質問3: 回答
合計サイズが 10 TB のリレーショナル データベースから BigQuery に何百万もの機密性の高い患者レコードをコピーするタスクがあります。ソリューションは、安全で時間効率が良いものでなければなりません。どのオプションが最適なソリューションですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
何百万もの機密性の高い患者記録を 10 TB のリレーショナル データベースから BigQuery にコピーするための最も安全で時間効率の高いオプションは、オプション B です。
このオプションでは、データベースからレコードを Avro ファイルとしてエクスポートし、そのファイルを Transfer Appliance にコピーして、Google に送信します。ファイルを受け取ったら、GCP Console の BigQuery ウェブ UI を使用して Avro ファイルを BigQuery に読み込むことができます。
この方法では、機密データをインターネット経由で送信する必要がなく、Transfer Appliance の高い転送速度に依存することで、セキュリティと速度の両方が確保されます。
オプション A では、gsutil を使用して Avro ファイルを GCS にアップロードし、BigQuery に読み込む必要がありますが、これは時間効率が悪く、セキュリティ上のリスクが生じる可能性があります。
オプション C では、レコードを CSV ファイルにエクスポートしますが、これは大量のデータを転送するための最も効率的な方法ではない可能性があり、オプション B よりも安全性の低いパブリック URL を使用します。
オプション D はオプション C と似ていますが、レコードを Avro ファイルとしてエクスポートするため、大量のデータを転送するためのより効率的な方法である可能性があります。ただし、Avro ファイルのパブリック URL を作成してストレージ転送サービスに依存することは、オプション B のように Transfer Appliance を使用するよりも安全性が低い場合があります。
</div></details>

 ### Q. 質問4: 回答
たとえば、100,000 個の異なるセンサーから BigQuery テーブルに分単位の解像度でデータを挿入する必要があるとします。集計された傾向のリアルタイム分析のニーズを満たすには、取り込みから 1 分以内にデータを使用できるようにする必要があります。データ量の大幅な増加が予想される場合、次のうちどのオプションを選択する必要がありますか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
このシナリオでは、B が最適な選択肢です。
Cloud Dataflow パイプラインを使用してセンサー データを BigQuery テーブルにストリーミングすると、質問で要求されているように、データの取り込みから 1 分以内に集計された傾向をリアルタイムで分析できます。さらに、パイプラインを使用すると、予想されるデータ量の大幅な増加に対応できます。
オプション A では、bq load コマンドを使用して 60 秒ごとにセンサー データのバッチをロードしますが、ストリーミングに比べて低速な方法であり、傾向のリアルタイム分析は提供されません。
オプション C では、60 秒ごとに INSERT ステートメントを使用してデータのバッチを挿入するため、大量のデータを処理するときにパフォーマンスの問題が発生する可能性があります。
オプション D は、既存のデータの更新には便利ですが、新しいデータの挿入には役立ちません
</div></details>

 ### Q. 質問5: 回答
BigQuery テーブルがあり、タイムスタンプと ID 列に基づいてデータをフィルタリングする WHERE 句を使用してクエリを実行する。ただし、bq query "-dry_run" コマンドを使用すると、フィルターがデータのごく一部しか選択しない場合でも、クエリによってテーブルのフル スキャンがトリガーされることがわかります。BigQuery でスキャンされるデータの量を最小限に抑えながら、SQL クエリをそのまま維持したい。どのアプローチを取るべきですか?
1. 　
2. 
3. 
4. 
...
パーティション テーブルは、パーティションと呼ばれるセグメントに分割された特別なテーブルであり、データの管理とクエリを容易にします。大きなテーブルを小さなパーティションに分割することで、クエリのパフォーマンスを向上させ、クエリによって読み取られるバイト数を減らすことでコストを制御できます。
https://cloud.google.com/bigquery/docs/clustered-tables
BigQuery のクラスタ化テーブルは、クラスタ化列を使用したユーザー定義の列の並べ替え順序を持つテーブルです。クラスタ化テーブルを使用すると、クエリのパフォーマンスが向上し、クエリのコストが削減されます。
</div></details>

 ### Q. 質問6: 回答
会社で履歴データを Cloud Storage にアップロードしたいが、セキュリティ ルールにより、外部 IP によるオンプレミス リソースへのアクセスが許可されていない。最初のアップロード後、既存のオンプレミス アプリケーションから新しいデータを毎日追加する予定です。データを安全に転送するにはどうすればよいですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
このシナリオに最適なオプションは、「gsutil rsync」コマンドを使用してオンプレミス サーバーと Cloud Storage を同期することです。
このアプローチにより、オンプレミスのリソースへの外部 IP アクセスを許可することなく、オンプレミスのサーバーから Cloud Storage にデータを安全に転送できます。
さらに、最初のアップロード後に毎日データの転送を自動化する方法を提供します。
オプション B の Dataflow を使用する場合、データを転送するためにカスタム パイプラインを記述する必要があり、このような単純なデータ転送タスクには適していない可能性があります。
オプション C の Dataproc ジョブ テンプレートの作成では、単純なデータ転送タスクに必要なリソースよりも多くのリソースと設定が必要になります。
オプション D の Compute Engine VM への FTP サーバーのインストールは、複雑さとセキュリティ上の懸念がさらに高まるため、最適な選択ではない可能性があります。また、「gsutil rsync」コマンドを使用するよりも多くの保守と管理が必要です。
</div></details>

 ### Q. 質問7: 回答
Cloud Storage に一連の履歴データが保存されており、それを分析する必要があります。無効なデータエントリを識別し、プログラミングやSQLの知識を必要とせずにデータ変換を実行できるソリューションが必要です。あなたならどのオプションを選びますか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
Cloud Dataprep は、プログラミングや SQL の知識がなくてもデータのクリーニングと変換に役立つ、強力で使いやすいデータ準備ツールです。
これにより、ユーザーはシンプルなインターフェイスと直感的な「レシピ」を使用して、データを視覚的に探索および変換できます。これらのレシピには、事前に構築された変換とエラー検出関数が含まれており、数回クリックするだけでデータに簡単に適用できます。
さらに、Cloud Dataprep では変更のプレビューがリアルタイムで提供されるため、ユーザーは変換の結果をすばやく確認できます。
また、Cloud Dataprep は Cloud Storage と統合されているため、Cloud Storage に保存されているデータに簡単にアクセスして処理できます。
これにより、データ移行が不要になり、時間とコストが削減されます。
そのため、Cloud Dataprep とレシピを併用すると、プログラミングや SQL の知識がなくても、無効なデータエントリを特定したり、Cloud Storage に保存された履歴データに対してデータ変換を実行したりできます。
</div></details>

 ### Q. 質問8: 回答
履歴アーカイブ用の生のソーシャルメディア投稿とそこから抽出されたデータの両方を保存し、最小限の手順で毎日何十万ものソーシャルメディア投稿を分析するという要件を満たすための最も効率的で費用対効果の高い方法は何ですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
履歴アーカイブ用の生のソーシャル メディア投稿とそこから抽出されたデータの両方を保存し、毎日何十万ものソーシャル メディア投稿を最小限の手順で分析するという要件を満たす最も効率的で費用対効果の高い方法は、未加工のソーシャル メディア投稿を Cloud Storage に保存し、抽出したデータを API から BigQuery に書き込むことです。
</div></details>

 ### Q. 質問9: 回答
有効な行キーを使用して Cloud Bigtable にデータを書き込むデータ パイプラインを使用している。目標は、パイプラインをモニタリングし、Cloud Bigtable クラスタのサイズをいつ増やすかを判断することです。そのためにできるアクションを2つ選んでください。次のオプションから 2 つ選択します。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
C: クラスターにノードを追加すると (レプリケーションではなく)、書き込みパフォーマンスが向上する可能性があります
参照:https://cloud.google.com/bigtable/docs/performance
D:Googleは、ストレージ使用率が70%>場合にノードを追加することを推奨しています
参照:https://cloud.google.com/bigtable/docs/modifying-instance#nodes
</div></details>
...
参照:https://cloud.google.com/bigtable/docs/replication-overview#use-cases
 ...
 ### Q. 質問12: 回答
3TB のリレーショナル データベースを Google Cloud Platform に移行する必要がありますが、このデータベースを使用するアプリケーションに大幅な変更を加えるためのリソースがありません。主な考慮事項は、データベースの運用コストです。この文脈では、データの保存と提供のためにGoogle Cloud Platformからどのサービスを選択しますか?次の選択肢があります。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
Cloud SQL は、MySQL、PostgreSQL、SQL Server データベースをサポートするフルマネージド サービスです。
クラウドでリレーショナルデータベースを実行および管理するためのシンプルで費用対効果の高いソリューションを提供します。
さらに、自動バックアップ、自動パッチ適用、高可用性などの機能を提供し、データベース管理の負担を軽減し、データベースの可用性を向上させることができます。
Cloud Spanner、Cloud Bigtable、Cloud Firestore などの他のオプションは高いスケーラビリティとパフォーマンスを提供しますが、アプリケーションに対してより大幅な変更が必要になる場合があり、2 TB のリレーショナル データベースにとって最も費用対効果の高いソリューションではない可能性があります。
</div></details>

 ### Q. 質問13: 回答
銀行に勤務する従業員は、ラベル付けされたデータセットを所有しており、承認されたローン申請に関する情報と、これらの申請が債務不履行に至ったかどうかに関する情報が含まれています。ここでのタスクは、将来の与信申請者のデフォルト率を予測できるモデルを作成することです。この目標を達成するために、どのような行動を取るべきですか?
1. 　
2. 
3. 
4. 
...
説明
予測モデルをトレーニングする:
所有しているラベル付きデータセットで予測モデルをトレーニングする必要があります。
このタスクに適したモデルは、ロジスティック回帰やデシジョン ツリーなど、ローン申請をデフォルトまたは非デフォルトとして分類できる分類モデルです。
モデルは、申請者の信用履歴、収入、雇用状況など、ローン申請のさまざまな機能を使用してトレーニングする必要があります
</div></details>

 ### Q. 質問14: 回答
不動産物件とそれに関連する特徴量 (緯度や経度など) のデータセットと、住宅価格を予測するニューラル ネットワークのトレーニングという目標がある場合、場所と価格の物理的な依存関係を組み込むには、どのようなアプローチを取る必要がありますか?次のうち、このタスクに最も適したオプションはどれですか
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
L1 正則化は、より影響力のある特徴量に高い重要性を割り当てる必要がある場合に使用します。
重要度の低い特徴量を 0 に縮小します。L2 正則化は、すべての入力特徴が出力に影響し、重みを持つすべての特徴が等しいサイズである場合に、より優れたパフォーマンスを発揮します。
参照:https://developers.google.com/machine-learning/crash-course/regularization-for-sparsity/l1-regularization
</div></details>

 ### Q. 質問15: 回答
あなたのチームは、社内のETLパイプラインの開発とメンテナンスを担当します。Dataflow ジョブの 1 つが入力データのエラーが原因で失敗しており、失敗したすべてのデータを再処理する機能など、パイプラインの信頼性を向上させる必要があります。推奨されるアプローチは何ですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
試してみる...catch ブロックでデータを変換する場合は、sideOutput を使用して、後で Pub/Sub に保存できる PCollection を作成します。
try...catch ブロックを DoFn に渡すと、ジョブはデータ変換中に発生したエラーを処理でき、エラーのあるデータはドロップされる代わりにサイド出力に渡すことができます。
これにより、パイプラインは残りのデータの処理を続行でき、エラーのあるデータを Pub/Sub トピックに保存して後で再処理することができます。
</div></details>

 ### Q. 質問16: 回答
組織のコスト感度を念頭に置きながら、オンプレミスの Hadoop クラスタから GCS 上の Dataproc に移行した後、シャッフル操作と Parquet 形式(それぞれ平均サイズ 200 〜 400 MB)の初期データを使用して、複雑な分析 Spark ジョブのパフォーマンスを最適化するにはどうすればよいでしょうか?Spark ジョブは現在、プリエンプティブルでないワーカーが <> つしかないプリエンプティブル VM で実行されています。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
C&Dは、費用対効果の高いソリューションの要件ではありません。そして、シャッフルが必要です。
https://cloud.google.com/dataproc/docs/support/spark-job-tuning#limit_the_number_of_files
</div></details>

 ### Q. 質問17: 回答
Kafka Connect プラグインをデプロイせずに、ウェブアプリケーションのログデータをオンプレミスの Apache Kafka クラスタから Google Cloud に複製して BigQuery と Cloud Storage で分析する場合、推奨される方法は何ですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
Kafka Connect プラグインをデプロイせずに、オンプレミスの Apache Kafka クラスタから Google Cloud にウェブアプリケーションのログデータを複製して BigQuery と Cloud Storage で分析する場合に推奨される方法は、GCE VM インスタンスに Kafka クラスタをデプロイし、トピックを GCE クラスタにミラーリングするようにオンプレミス クラスタを設定することです。次に、Dataproc クラスタまたは Dataflow ジョブを使用して、Kafka からの読み取りと GCS への書き込みを行います。
</div></details>

 ### Q. 質問18: 回答
プロジェクトごとに 2K の同時オンデマンド スロットの割り当てがあり、アカウントに新しいプロジェクトを追加しないようにしたい場合に、ユーザーが BigQuery スロットを取得できないことにどのように対処すればよいでしょうか?組織には、予算と優先順位の異なる複数の部署があります。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
BigQuery Reservations を使用するメリットは次のとおりです。
ワークロード管理。
スロットを購入したら、ワークロードに割り当てることができます。
これにより、ワークロードには、使用可能な BigQuery コンピューティング リソースの専用プールがあります。
同時に、ワークロードが割り当てられたスロットをすべて使用しない場合、未使用のスロットは他のワークロード間で自動的に共有されます。
集中購入: 組織全体にスロットを購入して割り当てることができます。BigQuery を使用するプロジェクトごとにスロットを購入する必要はありません
参照:https://cloud.google.com/bigquery/docs/reservations-intro
</div></details>

 ### Q. 質問19: 回答
比較のためにテーブルを結合する主キー列がないのに、元のジョブの出力を含むテーブルを読み込んでいる場合、元の ETL ジョブの出力と移行後の移行後の出力を比較するにはどうすればよいでしょうか。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
オプション C は、Dataproc クラスタと BigQuery Hadoop コネクタを使用して各テーブルからデータを読み取り、並べ替え後にテーブルのタイムスタンプ以外の列からハッシュを計算するため、最適なオプションです。各テーブルのハッシュを比較すると、テーブルに主キー列が含まれていなくても、内容が同一であることが保証されます
オプション A は、データの小さなサブセットしかチェックしないため、2 つのテーブルの内容を比較するための信頼できる方法ではありません。
オプション B は、異なるデータに対して同じハッシュ値が生成され、誤検知につながる可能性があるため、2 つのテーブルの内容を比較するための信頼できる方法ではありません。
オプションDは、OVER()関数を使用して層化ランダムサンプルを作成することを提案していますが、これらのサンプルを使用して2つのテーブルを比較する方法は明らかではありません。さらに、この方法では、データの代表的なサンプルが得られず、誤った比較につながる可能性があります。
</div></details>

 ### Q. 質問20: 回答
GCP を使用したハイブリッド デプロイで、GCP 上で動作するデータ転送サーバーへの並列アップロードを通じてデータセンターから Cloud Storage にインポートされた匿名化された顧客データに対して分析を行う場合、毎日の転送に時間がかかりすぎると経営陣から通知された場合、どうすれば転送速度を最大化できますか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
データセンターから Cloud Storage へのデータ転送速度を最大化するには、データセンターから GCP へのネットワーク帯域幅を増やす必要があります。
これにより、一度により多くのデータを転送できるようになり、毎日の転送が完了するまでにかかる時間が短縮されます。
サーバーの CPU サイズ(オプション A)やサーバー上の Google 永続ディスクのサイズ(オプション B)を増やしても、転送速度に大きな影響はありません。
Compute Engine から Cloud Storage(オプション D)にネットワーク帯域幅を増やすと、転送速度は向上する可能性がありますが、データセンターから GCP へのデータ転送のボトルネックは解決されません
</div></details>

 ### Q. 質問21: 回答
パイプラインのチェックポイント設定と分割を必要とする Apache Hadoop クラスターで実行する ETL パイプラインの作成を担当している場合、パイプラインの記述にはどの方法を使用する必要がありますか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
Pig は、パイプラインのチェックポイント設定と分割に使用できるスクリプト言語です
</div></details>

 ### Q. 質問22: 回答
トレーニングに数日かかっているニューラルネットワークモデルのトレーニング速度を上げるにはどうすればよいでしょうか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
トレーニングデータセットのサブサンプリングは、ニューラルネットワークモデルのトレーニング時間を短縮するための一般的な手法です。
データセットのより小さな部分を使用することで、処理に計算コストがかかるまでの反復回数が少なくて済むため、モデルのトレーニングを高速化できます。
テスト データセットのサブサンプリング (オプション A) は、パフォーマンス測定が不正確になり、結果の妥当性が損なわれる可能性があるため、お勧めしません。
入力特徴の数 (オプション C) またはニューラル ネットワークの層の数 (オプション D) を増やすことも、モデルのパフォーマンスの向上に役立ちますが、トレーニング速度とは直接関係ありません。
実際、モデルの複雑さを増すと、より多くの計算が必要になるため、トレーニング時間が長くなる可能性があります。
</div></details>

 ### Q. 質問23: 回答
期限切れになるすべてのログが正しくアーカイブされていると仮定して、特定の種類のデータへのアクセスの監査可能な記録を義務付ける政府の規制の対象となるデータを格納する推奨場所はどこですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
お客様には、規制コンプライアンスやデータ保護のために保持する必要がある大量のバルクデータがあるという声をよく耳にします。Archive クラスを Bucket Lock と組み合わせて使用すると、選択した期間、オブジェクトが変更されずに保持されます。これは、多くの業界、特にヘルスケアや金融サービスにおける法的保持要件を満たすのに役立ちます。
...
参照:https://cloud.google.com/bigquery/docs/share-access-views
 ...
 ### Q. 質問25: 回答
金融サービス会社は、頻繁に更新される50TBの金融時系列データをクラウドに保存しながら、常に新しいデータストリーミングに対応し、データ分析のために現在のApache Hadoopジョブをクラウドに移行するにはどうすればよいでしょうか?どのクラウド製品が彼らのニーズに最も適しているでしょうか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
Cloud Bigtable(オプション A)は、大規模なスケールと低レイテンシに最適化された NoSQL ワイドカラム データベース サービスです。
リアルタイム分析や時系列データに適しています
</div></details>

 ### Q. 質問26: 回答
Google Cloud 上の 10 TB データベース内の <> つのリレーショナル テーブルのストレージを設計して、水平方向にスケーラブルなトランザクションを可能にし、非キー列に対する範囲クエリのデータを最適化するにはどうすればよいでしょうか。次の選択肢からどのオプションを選択する必要がありますか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
オプション A は、ストレージに Cloud SQL を使用し、クエリ パターンをサポートするためにセカンダリ インデックスを追加することですが、Cloud SQL にはスケーラビリティの制限があるため、水平方向のスケーリングには不十分な場合があります。また、追加するインデックスが多すぎると、書き込みパフォーマンスに悪影響を及ぼす可能性があります。
オプション B のストレージに Cloud SQL を使用し、クエリ パターンをサポートするようにデータを変換する方法は、データの変換に追加の処理オーバーヘッドが必要になるため、最も効率的なソリューションではない可能性があります。
Cloud Spanner にはすでにスケーラビリティが組み込まれており、大規模なデータセットを高い同時実行性で処理できるため、ストレージに Cloud Spanner を使用し、クエリ パターンをサポートするようにデータを変換するオプション D は必要ない場合があります。ただし、クエリパターンをサポートするセカンダリインデックスを追加することは、依然として実行可能なソリューションです。
したがって、ストレージに Cloud Spanner を使用し、セカンダリ インデックスを追加してクエリ パターンをサポートするオプション C は、セカンダリ インデックスによるスケーラビリティと効率的なクエリを提供するため、最適なオプションです。
</div></details>

 ### Q. 質問27: 回答
入力データが CSV 形式の場合、複数のエンジンを使用して Cloud Storage 内のデータをクエリする複数のユーザーの集計値をクエリするコストを最小限に抑えるために、Google Cloud にデータ パイプラインをデプロイする場合、20 TB のテキスト ファイルのストレージをどのように設計する必要がありますか?どのストレージ サービスとスキーマ設計を利用する必要がありますか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
入力データが CSV 形式の場合、複数のエンジンを使用して Cloud Storage 内のデータをクエリする複数のユーザーの集計値をクエリするコストを最小限に抑えるために、データ パイプライン用に 20 TB のテキスト ファイルを Google Cloud に保存する場合の推奨ソリューションは、ストレージに Cloud Storage を使用し、クエリ用に BigQuery でデータを永続的なテーブルとしてリンクすることです。
</div></details>

 ### Q. 質問28: 回答
Google Cloud でユーザーのブログ投稿の件名ラベルを自動的に生成する必要があるアプリケーションを開発する場合、この機能を迅速に追加しなければならないという競争上のプレッシャーがあり、追加のデベロッパー リソースがなく、チームに機械学習の経験がないことを考えると、どのような行動を取るのが最善でしょうか?次のオプションのうち、最も適しているのはどれですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
Cloud Natural Language API は、エンティティ分析、感情分析、その他の自然言語処理タスクを実行できる、事前トレーニング済みの機械学習モデルです。
エンティティ分析機能を利用することで、API はテキストから関連するサブジェクト ラベルを自動的に識別して抽出できるため、追加の開発や機械学習の専門知識を必要とせずに、迅速かつ簡単に実装できるソリューションになります。
オプション B は、感情分析がテキストのトーンの分析に重点を置いており、件名ラベルの生成に適していない可能性があるため、お勧めしません。
オプション C と D には、TensorFlow を使用したテキスト分類モデルの開発とトレーニングに関する専門知識が必要ですが、開発者のリソースが限られていることと、機能を迅速に追加しなければならないという競争圧力を考えると、実現できない可能性があります
</div></details>

 ### Q. 質問29: 回答
Google のベスト プラクティスに基づいて、ANSI SQL クエリ、圧縮、入力場所からの並列読み込みをサポートする Google Cloud 上のデータ パイプラインで、非常に大きなテキスト ファイルのストレージを設計する場合、どのようなアプローチが推奨されますか?次のオプションのうち、最も適切なものはどれですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
Google のベスト プラクティスに基づいて、ANSI SQL クエリ、圧縮、入力場所からの並列読み込みをサポートする Google Cloud 上のデータ パイプラインで非常に大きなテキスト ファイルのストレージを設計する場合に推奨されるアプローチは、オプション B: Cloud Dataflow を使用してテキスト ファイルを圧縮された Avro 形式に変換し、データを Cloud Storage に保存することです。 クエリ用の BigQuery 永続リンク テーブルを使用します。
オプション A は、BigQuery をストレージとして使用すると追加費用が発生する可能性があり、Cloud Storage よりも拡張性が低いため、最適な選択ではありません。
オプション C は、gzip が BigQuery でのクエリ用に最適化された形式ではないため、推奨されません
Cloud Bigtable は ANSI SQL でのクエリ用に最適化されていない NoSQL データベースであるため、オプション D は最適な選択ではありません。
</div></details>

 ### Q. 質問30: 回答
一連の YouTube チャンネルから Google Cloud にログデータを転送して分析するソリューションをどのように設計すればよいでしょうか?グローバル マーケティング チームが最新の YouTube チャンネル ログ データに対して ANSI SQL やその他の種類の分析を実行できるように設計する必要があります。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
この質問は少しトリッキーなので、間違ったオプションを排除して答えを提供します。
「グローバルマーケティングチーム」というキーワードは、データへの世界的なアクセスを示唆しています。
この質問では、GCP 以外のターゲット プラットフォームは指定されていません。
BigQuery 転送サービスを使用する場合、データは BigQuery に転送されるだけで、他のターゲットはサポートされないため、さまざまなタイプの分析に対するチームの能力が制限されます。したがって、オプションCとDは除外されます。
すべてのオプションは、Google Cloud Storage(GCS)を指しています。
「世界規模」のアクセスの必要性と、特定の地域要件がないことを考慮すると、「複数地域」のストレージに言及しているオプションAが最も適切です。オプション B は、範囲が限定されることを意味する "地域" を参照しているため、あまり適していません。
宛先はGCSであり、マルチリージョンを持っているため、Aが利用可能な最良のオプションです。BigQuery Data Transfer Service は当初、Google 広告、キャンペーン マネージャー、Google アド マネージャー、YouTube などの Google アプリケーション ソースをサポートしていますが、bq データセット以外の宛先はサポートしていません
</div></details>

 ### Q. 質問31: 回答
このオンライン小売業者は、Google App Engine ベースのアプリケーションを使用しており、顧客が Google App Engine を使用して直接取引できるようにし、ビジネス インテリジェンス ツールを介してショッピング トランザクションを管理し、複数のデータセットからデータを分析できるようにしたいと考えています。この目的のためには、1 つのデータベースのみを使用することが優先されます。問題は、どのGoogle Cloudデータベースが最も適切なオプションであるかということです。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
したがって、BigQuery は大規模なデータセットに対して複雑な分析クエリを実行するために特別に設計されており、Google App Engine などの他の Google Cloud サービスと簡単に統合できるため、オンライン小売業者のニーズに最適なオプションです。
</div></details>

 ### Q. 質問32: 回答
ETLプロセスにApache Hadoopクラスタを利用する地震データ解析システムのアーキテクトは、一部のステップの計算が複雑なため、ETLプロセスでデータセットの処理に数日かかるという問題に遭遇します。問題を調査した後、センサーの調整手順が省略されていることがわかります。今後、センサのキャリブレーションを体系的に行うためには、次のうちどれをとればよいのでしょうか。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
今後、センサーのキャリブレーションを体系的に実行するには、ETL プロセスを変更して、センサーのキャリブレーションを生データに適用する新しい MapReduce ジョブを導入する必要があります。
この新しいジョブは、後続のすべての MapReduce ジョブがこの後にチェーンされるように、ETL プロセスの最初のジョブである必要があります。
これにより、すべてのデータが正確にキャリブレーションされ、その後の計算コストの高いジョブでデータをより効率的に処理できるようになります。
オプション A は、一部の transformMapReduce ジョブの後にセンサーのキャリブレーションを適用すると、データ分析が不正確になる可能性があるため、推奨されません。
オプションCは、ユーザーがセンサーキャリブレーションを手動で適用する必要があり、不整合やエラーにつながる可能性があるため、推奨されません。
オプション D は、出力の分散を予測し、正確でなく、効果的でない可能性がある補正を適用する必要があるため、推奨されません。
</div></details>

 ### Q. 質問33: 回答
組織はキャンペーンのデータ パイプラインを設定しており、すべての Google Cloud Pub/Sub ストリーミング データの入力とタイミングを定期的に特定する必要があります。これを実現するために、エンジニアは Google Cloud Dataflow のウィンドウ処理と変換を使用しています。ただし、テスト中に、すべてのストリーミング挿入で Cloud Dataflow ジョブが失敗するという問題が発生します。この問題の最も可能性の高い原因は何ですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
この質問では、エンジニアが Google Cloud Dataflow のウィンドウ処理と変換を使用して、キャンペーン中に入力とそのタイミングを定期的に識別することを決定したと記載されています。
ストリーミング データでは、ウィンドウ処理を使用してデータ ストリームを有限のウィンドウに分割し、データの各ウィンドウに対して計算を実行します。
ウィンドウ関数には、グローバルと非グローバルの 2 種類があります。
グローバル・ウィンドウ処理は、データ・ストリーム全体のすべてのデータ・エレメントに適用され、すべてのデータ・エレメントの到着時刻を考慮せずに均一に処理します。
一方、非グローバルウィンドウは、タイムスタンプに基づいて、異なるデータ要素に異なるウィンドウを適用します。
この質問では、Cloud Dataflow ジョブがすべてのストリーミング挿入で失敗することが記載されているため、エンジニアが非グローバル ウィンドウ関数を適用していない可能性があります。
非グローバル ウィンドウ関数がないと、ストリーミング データには有限の終了時刻がなく、ストリーミング データにグローバル ウィンドウを使用できないため、ジョブは失敗します。
参照:https://beam.apache.org/documentation/programming-guide/#windowing
</div></details>

 ### Q. 質問34: 回答
Firebase Analytics と Google BigQuery の統合から過去 30 日間のデータをクエリするには、app_events_YYYYMMDD形式の日次テーブルが自動的に作成され、クエリで従来の SQL を使用する必要がありますか?
1. 　
2. 
3. 
4. 
...
https://cloud.google.com/bigquery/docs/reference/legacy-sql
 ...
 ### Q. 質問35: 回答
来月、既存のアプリケーションが Google BigQuery のイベント データに接続できるようにするための 14 つの推奨アクションは何ですか?分析されるデータの大部分は、events_partitioned と呼ばれる時間分割されたテーブルにあります。クエリのコストを削減するために、過去 <> 日間のデータのみをクエリし、レガシ SQL で記述されている events という名前のビューが作成されました。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
C = 標準 SQL クエリは、レガシ SQL 構文を使用して定義されたビューを参照できません。
D = ODBC ドライバには、標準の Bigquery ロールを取得するサービス アカウントが必要です。
</div></details>
...
<details><div>
    答え：
説明
参照:https://cloud.google.com/bigquery/docs/reference/legacy-sql
</div></details>

 ### Q. 質問37: 回答
複数のアプリケーションサーバーからの入札イベントをリアルタイムで照合して、グローバルに分散したオークションアプリケーションでどのユーザーが最初に入札したかを判断するにはどうすればよいでしょうか。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
オプションA、共有ファイルを作成してApache Hadoopで処理する方法は、レイテンシーが増加し、データの一貫性に潜在的な問題が生じるため、リアルタイム処理には適していません。
オプション B では、各アプリケーション サーバーで入札イベントを Cloud Pub/Sub に書き込み、そのイベントを Cloud SQL に書き込むカスタム エンドポイントにプッシュしますが、Cloud SQL は高スループットのリアルタイム オペレーション用に設計されていないため、リアルタイム処理には適していません。
オプション C では、入札イベントを書き込むアプリケーションサーバーごとに MySQL データベースを設定し、各データベースに対して定期的にクエリを実行してマスターデータベースを更新する場合も、レイテンシーが発生し、一貫性の問題が発生する可能性があります。
したがって、各アプリケーション サーバーで入札イベントを Google Cloud Pub/Sub に書き込み、Google Cloud Dataflow でプル サブスクリプションを使用してイベントをリアルタイムでプルして処理し、最初に処理されたユーザーに入札を提供するオプション D は、分散型オークション アプリケーションでのリアルタイム入札処理に最も適したソリューションです。
</div></details>

 ### Q. 質問38: 回答
Google Cloud Storage の非公開情報を含む夜間のバッチ ファイルを含むデータ パイプライン プロセスを安全に自動化するにはどうすればよいでしょうか。バッチファイルは、Google Cloud Dataproc クラスタ上の Spark Scala ジョブで処理し、結果を Google BigQuery に格納する必要があります。次のうち、最も安全なアプローチはどれですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
ジョブを自動化する際には、特定のタスクを実行するために必要な最小限の特権を持つサービス アカウントを使用するのがベスト プラクティスです。
この場合、ジョブは Cloud Storage からバッチ ファイルを読み取り、結果を BigQuery に書き込む必要があります。
そのため、Cloud Storage バケットからの読み取りと BigQuery への書き込みが可能なサービス アカウントを作成し、そのサービス アカウントを使用してジョブを実行する必要があります。
参照:https://cloud.google.com/dataproc/docs/concepts/configuring-clusters/service-accounts#dataproc_service_accounts_2
</div></details>
...
正則化パラメーターを増やす: L1 や L2 正則化などの正則化パラメーターを増やすと、大きな重みとバイアスにペナルティを課し、モデルが最も関連性の高い特徴に焦点を合わせるようにすることで、過学習を制御するのに役立ちます。
 ...
 ### Q. 質問40: 回答
あなたは、何百万ものIoTデバイスからのテレメトリデータを処理するためにNoSQLデータベースを選択する任務を負っています。データ量は年間 100 TB の割合で増加しており、各データ エントリには約 100 個の属性があります。データ処理パイプラインは ACID 準拠を必要としませんが、高可用性と低遅延は重要です。分析のために、データ内の個々のフィールドをクエリする必要があります。この目的に適したデータベースを <> つ選びます。次のオプションから <> つ選択します。
</div></details>

 ### Q. 質問41: 回答
現在 20 時間あたり 000,4 ファイルで、今後 200 か月で 50 倍になると予想される、会社が生成するファイル数の増加が予想される場合、Google Cloud Platform でのデータ取り込みプロセスをどのように準備すればよいでしょうか。ファイルは CSV 形式で 10 KB 未満であり、取り込み後に処理する必要があります。現在のデータ インジェストの設定では、Google Compute Engine の仮想マシン上に安全な FTP(SFTP)サーバーがあり、ファイルの送信にはローカルの SFTP クライアントを使用していますが、セットアップが現在のボリュームにほとんど追いついていません。会社のサイトでは Google Cloud へのレイテンシが 00 ミリ秒で、インターネット接続帯域幅は <> Mbps に制限されています。目標は、毎日午前 <>:<> までにレポートを利用できるようにすることであり、インジェスト プロセスを改善するための <> つのアクションを提案するよう求められています。次のオプションがあります。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
A:サイズが小さいため、各ファイルを圧縮しても役に立ちません(実際、オーバーヘッドが増加する可能性さえあります)。
B:帯域幅は問題ではなく、増やす必要はありません。
C:-mでファイルを並行してアップロードすると、一般的に速度が向上します。
D: 個々の小さなファイルが多いと、各ファイルの処理とGCSへのアップロードにオーバーヘッドが加わり、GCSのアップロード速度はサイズに比例するため、問題になります。すべての小さなファイルを大きな単一のTARにパックすると、全体的なパフォーマンスが向上します。
E: Storage Transfer Service は数百 TB の移動を目的としており、最低でも 100 Mbps の接続が必要です (ドキュメントには、接続が 300 Mbps 未満の場合は gsutil を使用する方がよいと記載されています)。
参照:https://cloud.google.com/storage-transfer/docs/on-prem-overview#requirements
</div></details>

 ### Q. 質問42: 回答
Google Cloud 上のデータ パイプラインで JSON メッセージを書き込んで Cloud Pub/Sub から BigQuery に変換するサービスを選択し、サービス コストを最小限に抑え、手動による介入を最小限に抑えてサイズの異なる入力データ量に対応するには、どうすればよいでしょうか。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
オプション A は、ワーカー ノードの数のサイズを変更するために手動による介入が必要であり、最適に行わないと非効率性と余分なコストにつながる可能性があるため、最適な選択ではありません。
また、オプションBは、出力アーカイブを手動で分析してボトルネックを特定する必要があり、時間がかかり、常に最適なソリューションにつながるとは限らないため、最良の選択ではありません。
オプション D は、入力データ量を監視および収容する効率的な方法を提供しないため、最良の選択ではありません。デフォルト以外の Compute Engine マシンタイプを使用できますが、ワークロードに基づく自動スケーリングは提供されません。
オプション C は、Cloud Dataflow がワークロードに基づいて自動スケーリングを行うため、効率的で費用対効果が高いため、最適な選択肢です。さらに、Stackdriver でジョブ システムの遅延を監視することで、手動による介入なしに入力データ量を効率的に監視し、対応することができます
</div></details>

 ### Q. 質問43: 回答
Google Cloud のレコメンデーション エンジンを使用して、過去の視聴に基づいて新しい動画を顧客に表示し、顧客が視聴した動画内のエンティティのラベルを生成しながら、数 TB のデータに対する他の顧客の好みのデータに基づいて非常に高速なフィルタリングの提案を提供するアプリケーションを設計するには、どのようなアプローチが最適ですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
このシナリオに最適なアプローチは、Cloud Video Intelligence API を呼び出してラベルを生成し、Cloud Bigtable にデータを保存するアプリケーションを構築するオプション C です。
これにより、数TBのデータに対する他の顧客の好みからのデータに基づいて、迅速なフィルタリングの提案が可能になります。
オプション A と B には、Spark MLlib を使用した分類モデルの構築とトレーニングが含まれますが、このシナリオでは最も効率的なアプローチではない可能性があります。
オプション D では、データを Cloud SQL に保存する必要がありますが、これは非常に大規模なデータセットには最適ではない可能性があります。
</div></details>

 ### Q. 質問44: 回答
Kafka クラスタから Compute Engine インスタンスで実行されている Redis クラスタへのストリーミング データ挿入で、必要に応じて作成、ローテーション、破棄できる暗号鍵を使用して、保存データを暗号化するにはどうすればよいですか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明: 必要に応じて作成、ローテーション、破棄できる暗号化キーを使用して保存データを暗号化するには、Cloud Key Management Service(KMS)を使用する必要があります。具体的には、Cloud KMS 鍵を作成し、それを使用して Compute Engine インスタンスで実行されている Redis クラスタと Kafka クラスタのデータを暗号化する必要があります。
オプション A は、暗号化や Cloud KMS について言及していないため、正しくありません。また、Compute Engine インスタンス内のデータの参照も指しますが、これは保存データの暗号化とは直接関係ありません。
オプション C は、暗号化鍵をローカルで作成して Cloud KMS にアップロードすることを提案しており、ベスト プラクティスではないため、正しくありません。通常は、Cloud KMS で直接鍵を作成することをおすすめします。
オプション D は、保存データの暗号化とは関係のない API サービス呼び出しでキーを参照することを提案しているため、正しくありません。
</div></details>
...
また、ロジスティック回帰モデルにとって重要な、null 値を実数値変数として保持しながら null 値をモニタリングして調整する機能も備えているため、Cloud Dataprep を使用することをおすすめします。
 ...
 ### Q. 質問46: 回答
個々のユーザーが BigQuery に対して認証したり、データセットへの直接アクセスを許可したりすることなく、社内の IT アプリケーションのインターフェースから Google BigQuery に安全にアクセスするにはどうすればよいでしょうか。次のオプションのうち、どれを選択する必要がありますか?
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
個々のユーザーに BigQuery への認証を要求したり、データセットへの直接アクセスを許可したりすることなく、IT アプリケーションから Google BigQuery に安全にアクセスするための推奨されるアプローチは、サービス アカウントを作成し、そのアカウントへのデータセット アクセス権を付与することです。その後、サービス アカウントの秘密キーを使用してデータセットにアクセスできます。したがって、正しいオプションは C です。
オプション A では、ユーザーのグループを作成し、それらのグループにデータセットへのアクセス権を付与すると、個々のユーザー認証が必要になりますが、これは望ましくありません。
オプション B のシングル サインオン (SSO) プラットフォームとの統合は有効なアプローチですが、追加のセットアップと構成が必要であり、この場合は必要ない場合があります。
オプション D のダミー ユーザーを作成し、資格情報をファイルに格納することは、セキュリティ上の理由から推奨される方法ではなく、潜在的なデータ侵害につながる可能性があります
</div></details>
...
ウォーターマークとタイムスタンプを使用することで、パイプラインはオンタイム データと遅延データを区別し、それに応じて処理できます。これにより、パイプラインが正確かつ効率的な方法でデータを処理できるようになります。
 ...
 ### Q. 質問48: 回答
Google Cloud Storage のデータを使用して、週単位のワークロードを約 15 分で実行し、結果を Google BigQuery に出力する Google Cloud Dataproc 上の 30 ノードの Apache Spark クラスタを最適化して、コスト効率を高めるにはどうすればよいでしょうか。
1. 　
2. 
3. 
4. 
<details><div>
    答え：
説明
プリエンプティブル仮想マシンを使用することで、Google Cloud Dataproc で 15 ノードのクラスタを実行するコストを大幅に削減できます。
プリエンプティブル VM は通常の VM と似ていますが、低コストで利用できますが、Google が短期間(最大 24 時間)でシャットダウンできるという注意点があります。
ワークロードは毎週実行され、ジョブの実行時間は比較的短いため、プリエンプティブル VM の使用に適しています。
このシナリオでは、オプション A、C、D はコスト最適化には最適ではありません。
オプション A では、ワークロードを Google Cloud Dataflow に変換することを提案していますが、これは別のサービスであり、異なるスキルセットを必要とし、現在のワークロードには必要ない場合があります。
オプション C では、メモリ容量の大きいノードを使用することが提案されており、ジョブの実行時間は短縮される可能性がありますが、クラスターの実行コストも増加します。
オプション D では、ワーカー ノードに SSD を組み込んでジョブの実行時間を短縮することを提案していますが、これは 30 分で実行され、結果を Google BigQuery に出力できるジョブでは必要ない場合があります。
</div></details>
