Exam A
QUESTION 1 How can failback of a recovered virtual machine in vSphere Replication be performed?
...
QUESTION 2 What are three benefits of using a few large-capacity LUNs instead of many small-capacity LUNs? (Choose three.)
...
Explanation/Reference: Reference: https://docs.vmware.com/en/VMware-vSphere/6.5/com.vmware.vsphere.storage.doc/GUID2C8FAE1F-98F7-4E0B-AEA0-83A4FF16A252.html
QUESTION 3 After upgrading the vSphere infrastructure and VMware Tools to version 6.5, the “Upgrade VM Compatibility” option is grayed out in the vSphere Web Client. (See Exhibit.)
...
QUESTION 4 What availability mode does vCenter HA provide?
...
QUESTION 5 In which scenario will vSphere DRS balance resources when set to Fully Automated?
...
QUESTION 6 Which two configurable options are available in Boot Options for a virtual machine? (Choose two.)
...
QUESTION 7 What VMware technology can be used to move physical servers to the vSphere infrastructure?
...
QUESTION 8 Upgrading vCenter Server with Microsoft SQL database fails with the following error message:
...
QUESTION 9 What is a potential downside of having more than four paths per datastore?
...
QUESTION 10 Which feature facilitates the sharing of templates via vCenter Server?
...
QUESTION 11 A vSphere Administrator is in the process of configuring replication for a virtual machine, but is not able to select the Recovery Point Objective (RPO) value of 5 minutes.
...
8EA2D676B55A.html
QUESTION 12 What advanced parameter defines a custom isolation IP address for a vSphere HA cluster?
...
QUESTION 13 Which three VMware-recommended best practices should an administrator follow before upgrading to vSphere 6.5? (Choose three.)
...
QUESTION 14 What are two requirements to enable EVC in vSphere DRS cluster? (Choose two.)
...
QUESTION 15 Which method is supported for backing up the virtual machine that contains vCenter Server?
...
QUESTION 16 A vSphere Administrator must ensure that a user is able to view and manage the system configuration in vSphere Web Client.
...
QUESTION 17 What is the minimum number of hosts that must contribute capacity to a non-ROBO single site VMware vSAN cluster?
A. 1 B. 64 C. 3 D. 2
Correct Answer: C Section: (none) Explanation
Explanation/Reference: Explanation:
A minimum of three hosts must contribute capavity to non-ROBO single site.
QUESTION 18 What three conditions could result in VMware Flash Read Cache being unable to claim a disk? (Choose three.)
...
QUESTION 19 Boot from SAN is supported on which three storage technologies? (Choose three.)
...
QUESTION 20 Which two components are part of storage I/O multipathing on ESXi? (Choose two.)
...
QUESTION 21 Which quest operating system supports PVRDMA?
...
QUESTION 22 Which two statements are true about Highly Available vSphere Auto Deploy infrastructure? (Choose two.)
...
QUESTION 23 What is the correct way to replace the certificate on an existing vCenter Server that is part of a vCenter HA configuration?
...
QUESTION 24 The virtual machine log rotation size can be changed for all VMs by which two interfaces? (Choose two.)
...
QUESTION 25 Which two configuration options can exclude virtual disks from snapshot operations? (Choose two.)
...
QUESTION 26 The vSphere Data Protection performance analysis results in the “Conditionally Passed” status.
...
QUESTION 27 A vSphere Administrator wants to reserve 0.5 Gbps for virtual machines on each uplink on a disturbed switch that has 10 uplinks.
...
QUESTION 28 A vSphere administrator manages a cluster which includes critical and non-critical virtual machines. The cluster requires different permissions for contractors and non-contractors.
...
QUESTION 29 Which two storage configurations on the source server are unsupported with VMware Converter? (Choose two.)
...
QUESTION 30 Which two statements about vCenter HA are correct? (Choose two.)
...
QUESTION 31 An attempt to delete NFS datastore from a ESXi host returns the error:
...
QUESTION 32 A vSphere HA cluster with four hosts has admission control configured to tolerate one host failure.
...
QUESTION 33 Which command can be run to troubleshoot connectivity problems with vmkernal network interfaces?
...
QUESTION 34 An administrator finds that the Microsoft Exchange virtual machines are exhibiting network latency.
...
QUESTION 35 Which two types of iSCSI adapters require VMkernel networking configuration? (Choose two.)
...
QUESTION 36 What counter should an administrator check to determine if a virtual machine is ballooning?
...
QUESTION 37 Which two statements are true for NFS v4.1 firewall behavior? (Choose two.)
...
QUESTION 38 By default, how often is the vCenter vpx user password automatically changed?
...
QUESTION 39 Which metric in the Exhibit would identify high latency for a storage device?
...
QUESTION 40 What VM Monitoring setting would ensure that the failure interval for a virtual machine is defined as 120 seconds?
...
QUESTION 41 A vSphere Administrator attempts to enable Fault Tolerance for a virtual machine but receives the following error:
...
QUESTION 42 The business managers have asked the IT administrator to ensure configuration compliance in the vSphere infrastructure. The IT administrator wants to use Host Profiles to accomplish this, but is unable to do so.
...
QUESTION 43 What is the result of a “Consolidate” action on a virtual machine?
...
QUESTION 44 A Telco customer is implementing NFV using vSphere as the virtualization platform.
...
QUESTION 45 Storage capabilities be populated in what two ways? (Choose two.)
...
QUESTION 46 What is the VMware-recommended best practice for Fibre Channel zoning?
...
QUESTION 47 How can a vSphere administrator ensure that a set of virtual machines run on different hosts in a DRS cluster?
...
QUESTION 48 In network troubleshooting it is important to know what upstream switch the ESXi host is connected to.
...
QUESTION 49 vCenter Server Appliance Instance can be backed up using which Client Interface?
...
QUESTION 51 What tool can capture network traffic at the uplinks, or vSwitch, or virtual port levels?
...
QUESTION 52 vSphere DRS helps to balance virtual machines with available resources in a vSphere cluster.
...
QUESTION 53 Which three design requirements must be satisfied to enable RAID-6 objects in a VMware vSAN cluster? (Choose three.)
...
QUESTION 54 Which storage feature always requires the use of third-party software on ESXi?
...
QUESTION 55 Which is required for configuring iSCSI Software Adapter network port binding?
...
QUESTION 56 In vSphere Data Protection 6.1, which is the function of a Backup Verification job?
...
QUESTION 57 Which three requirements are needed to apply RAID 5 or RAID 6 erasure coding on a vSAN 6.2 or 6.5 object? (Choose three.)
...
QUESTION 58 An administrator has enabled vSphere Distributed Switch Health Check. The hosts connected to that Distributed Switch all display an alert: vSphere Distributed Switch MTU supported status
...
QUESTION 59 An administrator sees the following error in the log files:
...
QUESTION 60 Which Host Profile Subprofile configuration is used to configure firewall settings for ESXi hosts?
...
QUESTION 61 An administrator is attempting to access an ESXi host via the DCUI but is unable to do so. Which could be two possible causes? (Choose two.)
...
QUESTION 62 Where would a vSphere administrator find general vCenter Server logs on a vCenter Server Appliance?
...
QUESTION 63 Which two permissions are required to migrate a virtual machine with Storage vMotion? (Choose two.)
...
QUESTION 64 An administrator wishes to enable network port mirroring based on source port. Which feature should be enabled?
...
QUESTION 65 Which are three supported storage requirements for vCenter Server HA? (Choose three.)
...
Reference: https://docs.vmware.com/en/VMware-vSphere/6.5/com.vmware.vsphere.avail.doc/GUID8FD87389-8CC9-4298-8B08-A1526FB44524.html
QUESTION 66 Which are the two network requirements for a VMware vSAN 6.5 cluster? (Choose two.)
...
QUESTION 67 Which CLI command shows the physical uplink status for a vmnic?
...
QUESTION 68 Which three prerequisites must be in place prior to enabling secure boot for a virtual machine? (Choose three.)
...
QUESTION 69 Which is the maximum number of block devices (LUNs) that are supported by ESXi hosts that are running version 6.5?
...
QUESTION 70 A user is given the "Virtual machine user (sample)" role in a vSphere 6.x environment. Which three permissions would the user be able to perform? (Choose three.)
...
QUESTION 71 When performing a vCenter Server 5.5 for Windows with Microsoft SQL Server Express database migration to vCenter Server Appliance 6.5, which will be the target database?
...
QUESTION 72 Which three are supported upgrade processes for ESXi host? (Choose three.)
...
QUESTION 73 When enabling Storage DRS on a datastore cluster, which three components are enabled as a result?
(Choose three.)
...
QUESTION 74 An administrator has multiple vCenter Server 6.5 systems but cannot enable Storage I/O Control (SIOC) on a datastore shared by ESXi hosts managed by the vCenter Server systems.
...
QUESTION 75 What are two features introduced in Network I/O Control version 3? (Choose two.)
...
QUESTION 76 “OneAppServer” is a VM template stored in a content library named “LibraryOne”, but the vSphere administrator is not able to use this template for deployment.
...
QUESTION 77 A security officer has issued a new directive that users will no longer have access to change connected network adapters to limit denial of service on a virtual machine.
...
QUESTION 79 Which three statements are correct regarding booting an ESXi Host from Fibre Channel SAN? (Choose three.)
...
QUESTION 80 Which statement applies to the vSphere Replication appliance?
...
QUESTION 81 The storage vendor recommends adjusting the Fibre Channel HBA queue depth to resolve a performance problem.
...
QUESTION 82 Which statements regarding datastore clusters meets VMware’s recommended best practices?
...
QUESTION 83 Which two tasks should a vSphere administrator perform if the vmkernel log reports VMFS metadata errors? (Choose two.)
...
QUESTION 84 Which two statements are true for a vCenterServer user account that is used for vSphere Data Protection (VDP)? (Choose two.)
...
QUESTION 85 Which three storage protocols are supported by Virtual Volumes? (Choose three.) 
...
QUESTION 87 What VM Monitoring setting would ensure that the maximum reset window for a virtual machine is 24 hours?
...
QUESTION 88 What is the default space threshold for vSphere Storage DRS? 
A. 80% B. 70% C. 90% D. 95%
Correct Answer: A Section: (none) Explanation
...
QUESTION 89 Which is the correct sequence to upgrade a vSphere infrastructure? 
...
QUESTION 90 An administrator has a large boot from SAN cluster and wants to ensure consistent configuration by using host profiles.
...
QUESTION 91 When using VMware Converter to import a Windows server and improve the consistency of the destination virtual machine, which optional step can be performed immediately prior to powering down the source server? 
...
QUESTION 92 Restoration of the vCenter Server Appliance is accomplished using which interface? 
...
QUESTION 93 When attempting to migrate a virtual machine to another host in the same cluster, the error message, “Select a valid compute resource” returns.
...
QUESTION 94 Which three operations can cause VMFS metadata updates? (Choose three.) 
